...
WORKING_DIR = "/var/www/cc-mirror"
REPOS = [
    {"repo": "Clipteam/clipcc-desktop", "pre_release": True}
]
...
def do_download(remote_url: str, dst_file: Path, remote_ts: float, remote_size: int):
    ...
    with github_get(remote_url, stream=True, proxies={'http':'http://127.0.0.1:7890','https':'http://127.0.0.1:7890'}) as r:
        r.raise_for_status()
        tmp_dst_file = None
        try:
...
def main():
    ...
    for cfg in REPOS:
        ...
        versions = 3 # keep only one release
        ...
        for local_dir in working_dir.glob('*/*/*'):
            if local_dir.is_dir():
                try:
                    ...
                    pass
        f=open(os.path.join(WORKING_DIR, "info.json"), "w")
        f.write(json.dumps({"name": "Clipteam/clipcc-desktop", "last": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), "size": sizeof_fmt(total_size, suffix="")}))
        f.close()
        # os.chmod(os.path.join(WORKING_DIR, "LastSync"), 0o644)
        shutil.copy("/var/www/mirror-index/index.html", WORKING_DIR)
